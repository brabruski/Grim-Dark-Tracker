grimApp.controller('SpoilsWarController', ['$scope', '$firebaseArray', '$location', 'Config', 'DBServices', 'MaterialFunc', 'NewContentFactory', 'BattleFactory', '$timeout',
    function ($scope, $firebaseArray, $location, Config, DBServices, MaterialFunc, NewContentFactory, BattleFactory, $timeout) {
        ...
        $scope.checkZero = function (cardName) {
            var isTrue = BattleFactory.checkItemZero($scope.main.discards);
            if (!isTrue) {
                isTrue = BattleFactory.checkItemSecured(cardName);
            }
            return isTrue;
        };
        ...
        $scope.claimVP = function (ev) {
            MaterialFunc.confirmSteal(ev).then(function () {
                battleDetails[index] = BattleFactory.stealObjective(battleDetails[index]);
                battleDetails.$save(index);
                $scope.main.vicpoints = battleDetails[index].vicpoints;
                addSuccessMsg = 'Tactical Objective Successfully Stolen';
                $scope.showSimpleToast(addSuccessMsg);
            }, function () {
                addSuccessMsg = 'Steal Objective Claim Cancelled';
                $scope.showSimpleToast(addSuccessMsg);
            });
        };
